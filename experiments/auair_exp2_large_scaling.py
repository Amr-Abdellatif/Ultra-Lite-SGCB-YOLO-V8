...
def main() -> int:
    parser = argparse.ArgumentParser(description="AU-AIR Experiment 2: Small/X controlled scaling comparison")
    ...
    parser.add_argument("--exclude-x", action="store_true", help="Skip the UltraLite-X run.")
    ...
    if args.exclude_x:
        configs = [(path, name) for path, name in configs if name != "ultralite_x_ours"]
